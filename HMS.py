...
def payments(root):
    ...
    def getid(event=None):
        ...
        cur.execute("select * from payments where id = %s", (idd,))
        x = cur.fetchall()
        # print(x)
        cur.execute("select day,month,year,time,totalamt,r_n from paymentsf where id = %s", (idd,))
        yy = cur.fetchone()
        print(yy)
        # print (x)
        if (yy != None):
            ...
            l1 = Label(fr1, text='No Information Available', height=38, width=1080 - 308, font='msserif 15', bg='white',
                       fg='cyan4').pack()
            l1 = Label(fr2, text='No Information Available', height=38, width=1080 - 308, font='msserif 15', bg='white',
                       fg='cyan4').pack()
            l1 = Label(fr3, text='No Information Available', height=38, width=1080 - 308, font='msserif 15', bg='white',
                       fg='cyan4').pack()
            l1 = Label(fr4, text='No Information Available', height=38, width=1080 - 308, font='msserif 15', bg='white',
                       fg='cyan4').pack()
    ...
    nl = Label(b_frame, text='Made by Ashish Kumar', fg='black', bg='gray91', font='msserif 8')
...
def rooms(root):
    ...
    def roomdet(rno):
        ...
        cur.execute("select * from roomd where rn = %s", (rno,))
        rdata = cur.fetchall()
        for row in rdata:
            print(row[1])
        # print(rdata)
        ...
        Label(smf1, text=str(row[1]), fg='cyan4', bg='white', font='msserif 35').pack()
...
def staff(root):
    ...
    Label(emp1inf, text="Ashish Yadav", bg='white', fg="Grey", font='msserif 10').place(x=60, y=37)
    Label(emp1inf, text="Extention : 025", bg='white', fg="Grey", font='msserif 10').place(x=60, y=59)
    Label(emp1inf, text="Mail : ashishyadav@gmail.com", bg='white', fg="Grey", font='msserif 10').place(x=60, y=83)
    ...
    Label(emp1inf, text="Mail : Costoexe@seaview.com", bg='white', fg="Grey", font='msserif 10').place(x=45, y=83)
    emp1inf.pack_propagate(False)
    ...
    Label(emp1inf, text="Mail : Restaurant@seaview.com", bg='white', fg="Grey", font='msserif 10').place(x=72, y=83)
    emp1inf.pack_propagate(False)
    ...
    Label(emp1inf, text="Mail : Roomsserv@seaview.com", bg='white', fg="Grey", font='msserif 10').place(x=55, y=83)
    emp1inf.pack_propagate(False)
...
def reserve(root):
    ...
    def booking():
        if fn.get() == 'First Name' or ln.get() == 'Last Name' or cn.get() == 'Contact Number *' or em.get() == 'Email' or ad.get() == "Guest's Address" or noc.get() == 'Number of Children' or noa.get() == 'Number of Adults' or nod.get() == 'Number of Days of Stay' or roomn.get() == 'Enter Room Number':
            ...
            cur.execute("select rstatus from roomd where rn = %s", (roomn.get(),))
            temp = cur.fetchone()
            if temp[0] == 'Reserved':
                ...
                # global pmethod
                cur.execute("select price from roomd where rn = (%s)", (roomn.get(),))
                rp = cur.fetchone()
                print(rp)
                ...
                def f():
                    if pmethod != '':
                        ...
                        print(cid)
                        print(pmethod.get())'''
                        cur.execute("insert into paymentsf(f_name,l_name,c_number,email,r_n,day,month,year,time,"
                                    "method,totalamt) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
                                        fn.get(), ln.get(), cn.get(), em.get(), roomn.get(), str(now.strftime("%d")),
                                        str(now.strftime("%b")), str(now.strftime("%Y")), str(now.strftime("%H:%M")),
                                        str(pmethod.get()), amtpd))
                        cur.execute("update roomd set rstatus='Reserved' where rn = %s ", (roomn.get(),))
                ...
                Button(payroot, text='Pay', font='msserif 12', bg='Green', fg='White', width=28, command=f).place(x=0,
                                                                                                                  y=200)
    ...
    def unreserve():
        if (roomn.get() == 'Enter Room Number') or (roomn.get() == ''):
            ...
            cur.execute("update roomd set rstatus='Unreserved' where rn = %s ", (roomn.get(),))
    ...
    def findrooms():
        cur.execute(
            'select rn,price,rstatus from roomd where beds = %s and ac = %s and tv = %s and internet = %s order by price asc',
    ...
    unres = Button(b_frame, text='Unreserve', bg='white', fg='cyan4', font='timenewroman 11', activebackground='green',
                   command=unreserve).place(x=327, y=270)
    findrooms = Button(b_frame, text='Find Rooms', bg='white', fg='cyan4', font='timenewroman 9',
                       activebackground='green',
                       command=findrooms).place(x=830, y=155)
...
def omega(p):
    root = tk.Tk()
    root.title("Hotel SeaView")
    root.minsize(height=550, width=1080)
    ...
    tf_label = Label(top_frame, text='Hotel SeaView', font='msserif 33', fg='dark blue', height=70)
    ...
    path = "room.ico"
    img = ImageTk.PhotoImage(Image.open(path))
    b1 = Button(sl_frame, image=img, text='b1', bg='white', width=180, command=lambda: rooms(root))
    b1.image = img
    b1.place(x=180, y=0)
    path2 = "status.ico"
    img1 = ImageTk.PhotoImage(Image.open(path2))
    b2 = Button(sl_frame, image=img1, text='b2', bg='white', width=180, command=lambda: hotel_status(root))
    b2.image = img1
    b2.place(x=0, y=0)
    path3 = 'contact.ico'
    img3 = ImageTk.PhotoImage(Image.open(path3))
    b3 = Button(sl_frame, image=img3, text='b2', bg='white', width=180, command=lambda: staff(root))
    b3.image = img3
    b3.place(x=180 * 4, y=0)
    path4 = 'payment.ico'
    img4 = ImageTk.PhotoImage(Image.open(path4))
    b4 = Button(sl_frame, image=img4, text='b2', bg='white', width=180, command=lambda: payments(root))
    b4.image = img4
    b4.place(x=180 * 3, y=0)
    path5 = 'exit.ico'
    img5 = ImageTk.PhotoImage(Image.open(path5))
    b5 = Button(sl_frame, image=img5, text='b2', bg='white', width=180, height=100, command=lambda: login(root))
    b5.image = img5
    b5.place(x=180 * 5, y=0)
    path6 = 'reserve.ico'
    img6 = ImageTk.PhotoImage(Image.open(path6))
    b6 = Button(sl_frame, image=img6, text='b2', bg='white', width=180, height=100, command=lambda: reserve(root))
    b6.image = img6
